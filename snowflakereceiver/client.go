...
var (
    // identifierRegex validates table/column names to prevent SQL injection
...
// ============================================================================
// UPDATED ROW STRUCTS WITH ALL DIMENSIONS
// ============================================================================
...
type loginHistoryRow struct {
    ...
    reportedClientType sql.NullString  // ⭐ NEW
    loginCount         int64
}
...
type taskHistoryRow struct {
    ...
    warehouseName     sql.NullString  // ⭐ NEW
    errorCode         sql.NullString  // ⭐ NEW
...
type replicationUsageRow struct {
    databaseName      sql.NullString
    sourceAccount     sql.NullString  // ⭐ NEW
    targetAccount     sql.NullString  // ⭐ NEW
    sourceRegion      sql.NullString  // ⭐ NEW
    targetRegion      sql.NullString  // ⭐ NEW
    totalCredits      float64
...
type eventTableRow struct {
    ...
    resourceName  sql.NullString  // ⭐ NEW
    userName      sql.NullString  // ⭐ NEW
    databaseName  sql.NullString  // ⭐ NEW
    schemaName    sql.NullString  // ⭐ NEW
...
func newSnowflakeClient(logger *zap.Logger, config *Config) (*snowflakeClient, error) {
    ...
    // Create cardinality tracker with limits
    cardLimits := DefaultCardinalityLimits()
    // Override with config if provided
    if config.MaxUsersCardinality > 0 {
        cardLimits.MaxUsers = config.MaxUsersCardinality
    }
    if config.MaxSchemasCardinality > 0 {
        cardLimits.MaxSchemas = config.MaxSchemasCardinality
    }
    if config.MaxDatabasesCardinality > 0 {
        cardLimits.MaxDatabases = config.MaxDatabasesCardinality
    }
    ...
    return &snowflakeClient{
        ...
        config:      config,
        rateLimiter: limiter,
        cardTracker: newCardinalityTracker(logger, cardLimits),
    }, nil
}
...
// ============================================================================
// QUERY EXECUTION WITH RETRY & RATE LIMITING
// ============================================================================
...
func (c *snowflakeClient) queryMetrics(ctx context.Context) (*snowflakeMetrics, error) {
    ...
    if c.config.Metrics.QueryHistory.Enabled {
        if err := c.queryQueryStats(ctx, metrics); err != nil {
...
// ============================================================================
// CURRENT QUERIES (INFORMATION_SCHEMA - REAL-TIME)
// ============================================================================
...
func (c *snowflakeClient) queryCurrentQueries(ctx context.Context, metrics *snowflakeMetrics) error {
    query := `
        ...
        GROUP BY WAREHOUSE_NAME, WAREHOUSE_SIZE, QUERY_TYPE, EXECUTION_STATUS, 
                 DATABASE_NAME, SCHEMA_NAME, USER_NAME, ROLE_NAME, ERROR_CODE
        LIMIT ?
    `
...
// ============================================================================
// WAREHOUSE LOAD (INFORMATION_SCHEMA - REAL-TIME)
// ============================================================================
...
// ============================================================================
// QUERY STATS (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================

func (c *snowflakeClient) queryQueryStats(ctx context.Context, metrics *snowflakeMetrics) error {
    query := `
        SELECT 
            WAREHOUSE_NAME,
            WAREHOUSE_SIZE,
    ...
    rows, err := c.queryWithRetry(ctx, "query_stats", query, c.config.GetMaxRowsPerQuery())
    if err != nil {
        return fmt.Errorf("failed to query query stats: %w", err)
    }
    defer rows.Close()
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(
            ...
            return fmt.Errorf("failed to scan query stat row: %w", err)
        }
...
// ============================================================================
// CREDIT USAGE (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
// ============================================================================
// STORAGE USAGE (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
// ============================================================================
// LOGIN HISTORY (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
func (c *snowflakeClient) queryLoginHistory(ctx context.Context, metrics *snowflakeMetrics) error {
    query := `
        ...
        GROUP BY USER_NAME, IS_SUCCESS, ERROR_CODE, REPORTED_CLIENT_TYPE
        LIMIT ?
    `
...
// ============================================================================
// PIPE USAGE (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
// ============================================================================
// DATABASE STORAGE (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
// ============================================================================
// TASK HISTORY (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
func (c *snowflakeClient) queryTaskHistory(ctx context.Context, metrics *snowflakeMetrics) error {
    query := `
        SELECT 
            ...
            AVG(TIMESTAMPDIFF(second, SCHEDULED_TIME, COMPLETED_TIME)) as AVG_EXECUTION_TIME
        FROM SNOWFLAKE.ACCOUNT_USAGE.TASK_HISTORY
        WHERE QUERY_START_TIME >= DATEADD(hour, -1, CURRENT_TIMESTAMP())
        GROUP BY DATABASE_NAME, SCHEMA_NAME, NAME, STATE, QUERY_START_TIME, ERROR_CODE
        LIMIT ?
    `
    ...
    for rows.Next() {
        ...
        var queryStartTime sql.NullTime
        if err := rows.Scan(
            &row.databaseName,
            &row.schemaName,
            &row.taskName,
        ...
        // Extract warehouse name from task metadata if available
        // Note: TASK_HISTORY doesn't directly provide warehouse name
        // This would need to be enhanced with a JOIN to TASKS view
...
// ============================================================================
// REPLICATION USAGE (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
// ============================================================================
// AUTO-CLUSTERING HISTORY (ACCOUNT_USAGE - HISTORICAL)
// ============================================================================
...
func (c *snowflakeClient) queryAutoClusteringHistory(ctx context.Context, metrics *snowflakeMetrics) error {
    ...
    rows, err := c.queryWithRetry(ctx, "auto_clustering_history", query, c.config.GetMaxRowsPerQuery())
    if err != nil {
...
func (c *snowflakeClient) queryEventTableLogs(ctx context.Context, metrics *snowflakeMetrics, eventType string) error {
    ...
    query := fmt.Sprintf(`
        SELECT 
            ...
            SEVERITY_TEXT,
            RESOURCE_ATTRIBUTES['snow.resource.name']::STRING as RESOURCE_NAME,
    ...
    if err != nil {
        return fmt.Errorf("failed to query event table logs for %s: %w", eventType, err)
    }
    defer rows.Close()
    ...
    for rows.Next() {
        ...
        // Append to appropriate slice based on event type
        switch eventType {
        case "QUERY":
            metrics.eventQueryLogs = append(metrics.eventQueryLogs, row)
        case "TASK":
            metrics.eventTaskLogs = append(metrics.eventTaskLogs, row)
        case "FUNCTION":
            metrics.eventFunctionLogs = append(metrics.eventFunctionLogs, row)
        case "PROCEDURE":
            metrics.eventProcedureLogs = append(metrics.eventProcedureLogs, row)
        }
    }
...
func (c *snowflakeClient) queryOrgCreditUsage(ctx context.Context, metrics *snowflakeMetrics) error {
    ...
    orgNameCol := c.config.Organization.OrganizationNameColumn
    if orgNameCol == "" {
        orgNameCol = "ORGANIZATION_NAME"
    }
    accountNameCol := c.config.Organization.AccountNameColumn
    if accountNameCol == "" {
        accountNameCol = "ACCOUNT_NAME"
    }
    ...
    query := fmt.Sprintf(`
        ...
        WHERE USAGE_DATE >= DATEADD(day, -1, CURRENT_DATE())
        GROUP BY %s, %s, SERVICE_TYPE
        LIMIT ?
    `, orgNameCol, accountNameCol, orgNameCol, accountNameCol)
...
func (c *snowflakeClient) queryOrgDataTransfer(ctx context.Context, metrics *snowflakeMetrics) error {
    ...
    query := fmt.Sprintf(`
        ...
        WHERE USAGE_DATE >= DATEADD(day, -1, CURRENT_DATE())
        GROUP BY %s, SOURCE_ACCOUNT_NAME, TARGET_ACCOUNT_NAME, SOURCE_REGION, TARGET_REGION
        LIMIT ?
    `, orgNameCol, orgNameCol)
...
func (c *snowflakeClient) executeCustomQuery(ctx context.Context, metrics *snowflakeMetrics, query *CustomQuery) error {
    // Validate query SQL (basic check - not comprehensive)
    if query.SQL == "" {
        return fmt.Errorf("custom query %s has empty SQL", query.Name)
    }
    ...
    if err := rows.Err(); err != nil {
        return fmt.Errorf("error iterating custom query rows: %w", err)
    }
    
    metrics.customQueryResults = append(metrics.customQueryResults, result)
    return nil
}
...
// getCardinalityStats returns current cardinality statistics for monitoring
func (c *snowflakeClient) getCardinalityStats() map[string]int {
    return c.cardTracker.getStats()
}

// getDroppedCounts returns counts of dropped dimensions due to circuit breakers
